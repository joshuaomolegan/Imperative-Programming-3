...
class AppFrame(map: RoadMap, app: AutoSnail) 
    ...
    private val changeColourButton = Button("Change Colour"){
        ColorChooser.showDialog(viewer, "Select a colour", new Color(127, 225, 255)) match {
            case Some(color) => viewer.changeBackground(color)
            case _ =>
        }}
    ...
    private val controls = new GridBagPanel {
        val elements = Array[Component](new MyLabel("From:"), fromText,
            ...
            changeColourButton, // Add changeColorButton to the array of components
            delaySlider)
